...
void sEE_WriteStatusRegister(uint8_t regval) {
    ...
    EEPROM_SendInstruction((uint8_t *) command, 2);
...
void EEPROM_SendInstruction(uint8_t *instruction, uint8_t size) {
    ...
    if (HAL_SPI_Transmit(&EEPROM_SPI, (uint8_t *) instruction, (uint16_t) size, EEPROM_Timeout) != HAL_OK) {
        EEPROM_ErrorHandler();
    }
}
...
EepromOperations EEPROM_WritePage(uint8_t *pBuffer, uint16_t WriteAddr, uint16_t NumByteToWrite) {
    ...
    // send commands in one packet of 3 bytes
    uint8_t header[3];
    ...
    EEPROM_SendInstruction((uint8_t *) header, 3);


    // Make 5 attemtps to write the data
    for (uint8_t i = 0; i < 5; i++) {
        spiTransmitStatus = HAL_SPI_Transmit(&EEPROM_SPI, pBuffer, NumByteToWrite, EEPROM_Timeout);
